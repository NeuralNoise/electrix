...
{
    $filename = SITE_PATH . "/temp/debug/debugLog-".date("d.m.Y").".log";
     // open file
    $fd = fopen($filename, "a");
    // write string
...
function loadClass($class_name) {
    require_once SITE_PATH.'/model/'.$class_name.'.class.php';
}

function loadException($class_name) {
    require_once SITE_PATH.'/model/exceptions/'.$class_name.'.class.php';
}
...
function checkForCache( $cacheFile , $expireTime )
{
	...
	$cacheFile = SITE_PATH . '/temp/cache/'.$cacheFile.'.txt';
